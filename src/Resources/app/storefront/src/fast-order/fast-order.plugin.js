...
export default class FastOrderPlugin extends Plugin {
            ...
            for(let ev = 0; ev < events.length; ev++) {
                inputArticle[i].addEventListener(events[ev], (event) => {
                    ...
                    if(searchValue == ''){
                        ...
                        this._client.get('/fast-order/articles/' + encodeURIComponent(searchValue), this.handleData.bind(this))
                    }
                })
            }
        }
    ...
    quantityEvents() {
        const inputQuantity = document.querySelectorAll('.fast-order-input-quantity')
    ...
    calculateTotal() {
        ...
        for(let i = 0; i < inputArticle.length; i++) {
            if(inputArticle[i].getAttribute('data-price') && inputQuantity[i].value.trim()) {
